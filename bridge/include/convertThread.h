...
/** \class ConvertThread
   ...
   */
class ConvertThread : public yarp::os::RateThread
{

protected:
  std::string name; /**< The name of the module */
  std::string yarpPort; /**< The YARP port from which we'll be seeking data */
  std::string rosTopic; /**< The name of the ROS topic we want to publish on */
...
public:
  ...
  /**
     ...
     * \param _fd : The file descriptors for the pipe 
     */
  ConvertThread(std::string _name, std::string _yarpPort, std::string _rosTopic, int _rate, int *_fd);
