...
private:
  yarp::os::BufferedPort<yarp::os::Bottle> inPort; /**< The port connected to the device port to gather data */
  field yarpGroups; /**< The headers corresponding to the data we are about to send */
  std::string headers; /**< Headers are all converted to one big string in order to be sent through the pipe */
...
*/
  DataThread(std::string _name, std::string _yarpPort, std::string _rosTopic, field _yarpGroups, int _rate, int *_fd);
